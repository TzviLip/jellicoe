...
export default async function DoctorInbox() {
  ...
  if (roleData?.role !== 'doctor') redirect('/login')

  const { data: submissions } = await supabase
    .from('patient_submissions')
    .select('id, created_at, full_name, id_number, status, consultation_type, date_of_birth, report_html, finalised_at')
    .in('status', ['pending_doctor', 'complete'])
    .order('created_at', { ascending: false })

  const ready    = submissions?.filter(s => s.status === 'pending_doctor') ?? []
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Welcome back, {roleData?.name ?? 'Doctor'}</p>
        </div>
      ...
      {ready.length > 0 ? (
        <section className="mb-10">
          <div className="flex items-center gap-3 mb-4">
            <h2 className="text-base font-semibold text-slate-700">Ready for your review</h2>
          ...
          <div className="space-y-3">
            {ready.map(s => (
              <Link
                key={s.id}
                href={`/doctor/patients/${s.id}`}
                className="block bg-white border-2 border-blue-200 rounded-2xl px-6 py-5
        ...
        <div className="mb-10 bg-green-50 border border-green-100 rounded-2xl px-6 py-8 text-center">
          <p className="text-green-700 font-medium">No reports awaiting review.</p>
        </div>
      )}
      ...
      {complete.length > 0 && (
        <section>
          <h2 className="text-base font-semibold text-slate-700 mb-4">Completed reports</h2>
