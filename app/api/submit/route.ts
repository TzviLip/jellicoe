...
// Limits each IP to 5 submissions per 10 minutes
...
function validateSubmission(data: Record<string, unknown>): ValidationError[] {
  ...
  // Consultation type must be a non-empty array of known values
  const validTypes = [
  ...
  // Additional risks must be an array
  ...
  // Sanitise free-text fields — reject suspiciously long inputs
  const textFields = ['fragilityFractures', 'vertebralFractures', 'ethnicity', 'sex']
...
export async function POST(req: NextRequest) {
  try {
    ...
    // Read radiographer email from notification_config (not env var)
    ...
    const radiographerEmails: string[] = config?.recipient_emails ?? []

    // Fallback to env var if DB not yet configured
    if (radiographerEmails.length === 0 && process.env.RADIOGRAPHER_EMAIL) {
